...
export async function execute(oldState: VoiceState, newState: VoiceState): Promise<void> {
	...
	const leftQueue = oldState.channelId === queueChannelId && newState.channelId !== queueChannelId;
	const joinedQueue = newState.channelId === queueChannelId && oldState.channelId !== queueChannelId;
